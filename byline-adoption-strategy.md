...
Maximize adoption of Byline (bylinespec.org), an extension vocabulary for RSS, Atom, and JSON Feed, across the WordPress ecosystem by building a single standalone plugin that outputs Byline-structured feed data regardless of which multi-author plugin (if any) a site uses.
...
The Byline extension vocabulary (v0.1.0, January 2026) has zero implementations and zero feed reader support. Adoption requires both supply (feeds emitting Byline data) and demand (readers parsing it). A WordPress plugin that "just works" for tens of thousands of multi-author sites lowers the supply-side barrier to zero. The pitch to feed reader developers then becomes: "here are real feeds already emitting Byline data — here's what you could do with it in your UI."
...
**The perspective/velocity pitch to Current.** Godier's Current RSS reader assigns velocity (content aging speed) per *source*: Breaking, News, Article, Essay, or Tutorial. Byline's `perspective` element enables velocity per *item* — a single source's posts can age at different rates based on the publisher's perspective signal. The mapping is natural: `reporting` → Breaking/News velocity, `analysis` → Article velocity, `personal` → Essay velocity, `tutorial` → Tutorial/Evergreen velocity. This gives Current item-level display intelligence without requiring subscriber configuration per source. The pitch: "here are WordPress feeds already emitting perspective data — here's how Current could use it to auto-assign velocity per item instead of per source." See `first-implementor-notes.md § 3` for the full perspective-to-velocity mapping table and spec feedback.

### Phase 4: iterate on the spec

As a first implementor, provide feedback on the spec based on real-world implementation experience. Concrete findings from this implementation are documented in `first-implementor-notes.md` and include:

- **Role ambiguity with multiple authors.** The standalone `byline:role` element is ambiguous when multiple `byline:author ref` elements are present. Proposed fix: add `role` as an attribute on `byline:author`, e.g., `<byline:author ref="jdoe" role="creator"/>`.
- **No canonical JSON Feed mapping.** The spec says it extends JSON Feed but doesn't define the extension shape. Proposed: a `_byline` extension object on standard `authors` entries. Reference implementation in `inc/feed-json.php`.
- **The contributor pre-pass pattern.** Every feed generator needs to collect unique authors before starting output. This should be an implementor's note in the spec.
- **Channel-level perspective default.** Single-purpose sites shouldn't need to set perspective on every post. The spec should document a channel-level default with per-item override inheritance.
...
| Feed output (RSS2 + Atom + JSON Feed) | ~350 lines |
