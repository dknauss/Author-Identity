...
│   ├── feed-json.php                   # JSON Feed Byline output (_byline extensions + /feed/json endpoint)
...
7. JSON Feed output at `/feed/json` includes `_byline` extension objects on `authors` entries and item-level `_byline.perspective` when set.
8. Standard `<author>` and `<dc:creator>` elements are preserved (Byline is additive).
