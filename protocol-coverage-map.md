...
- **WordPress status:** Not in core. Byline extends JSON Feed via `_byline` extension objects. WordPress implementation now included in WP-02.
...
- **WordPress status:** Themes can output. Byline Feed will enable via profile link output. The Byline extension's `byline:profile` elements can carry `rel` attributes for this purpose.
...
**Content perspective.** Only Byline carries it. No other protocol has a concept of editorial intent (reporting vs. opinion vs. satire). This is Byline's most distinctive contribution — it addresses a problem (content collapse in feeds) that no other standard even recognizes.
...
- [byline-spec-plan.md](byline-spec-plan.md) — Byline extension assessment
