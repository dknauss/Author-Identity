...
│  Core WP fallback · later: HM / Molongui        │
└─────────────────────────────────────────────────┘
```
...
(object) [
    ...
    'ap_actor_url' => string,  // ActivityPub actor URL when confidently resolvable.
                               // Optional cross-cutting field for WP-04/WP-05 output.
                               // Distinct from declared social profile links.
...
On `plugins_loaded`, the plugin checks for active multi-author plugins and loads the first matching implemented adapter:
...
| 02 | Byline RSS2, Atom, and JSON Feed output | 01 | Feeds include valid Byline namespace, contributors, per-item author refs |
| 03 | Perspective meta field and editor UI | 01, 02 | Editors set perspective per post, appears in feed output |
| 04 | `fediverse:creator` meta tag output | 01 | Shared links on Mastodon show author bylines; `ap_actor_url` populated when available |
...
Gate D ─── Rights expansion (after B'/C as applicable) ──────────────
           WP-06 begins only after WP-04, WP-05, and HM Authorship.
           Its HTML/header signals can ship after B' because they
           already have deployed consumers. Feed-level rights
           metadata (`cc:license` and any dedicated deny-policy
           extension) still waits for Gate C because it depends on
           readers parsing Byline data.
```

**Rationale for this sequence.** The original four-gate sequence treated WP-04/05/06 as a single block gated on reader-side signal. The current plan separates two different questions:

1. which work best proves the adapter model next
2. which rights outputs depend on reader-side Byline support

That yields a more disciplined order:
